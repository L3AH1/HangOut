...
export default class Itinerary extends React.Component{
  ...
  componentDidMount() {
    ...
    const data = { //Recupération des données dans l'url
  ...
  launchItinerary = (data) => {
    ...
    let villeDepart = {coords : {lat : parseFloat(villeDep[0]), lon : parseFloat(villeDep[1])}},
        villeDArrivee = {coords : {lat : parseFloat(destination[0]), lon : parseFloat(destination[1])}};
    ...
    let callbacks = {
      onSuccess : (result) => {
        ...
        for (let i = 0; i < output.length && result.header.summaries.length; i++) {
              ...
              <div className="relative">
                <div className="rounded-lg px-2 bg-white border shadow-2xl">
                  <div className="px-4 py-8 mb-4">
                    <div className="flex gap-8 flex-row-reverse flex items-center pr-56"></div>
                    <div>
