...
export default function getcout(data) {
  ...
  for (var i = 0; i < data.itinerary.length; i++) {
    if (data.itinerary[i].duree == "0h00") {
      ...
      const depart = formattageVille(
        data.itinerary[i].destinationDepart.split(" (")[0]
      );
      const arrivee = formattageVille(
        data.itinerary[i].destinationArrivee.split(" (")[0]
      );
      if (depart == arrivee) {
        const toPush = {
          id: 0,
        ...
        const URL =
          "https://ressources.data.sncf.com/api/records/1.0/search/?dataset=tarifs-tgv-par-od&facet=od&q=" +
          depart +
          "-+" +
          arrivee;
        const toPush = {
